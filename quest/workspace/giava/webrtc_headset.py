...
# Upstream defaulted these to bare filenames, i.e. the process working
# directory. Here they are mounted read-only into the container (see
# quest/docker-compose.yml) and QUEST_SECRETS_DIR points at them, so a script
# run from any directory finds them.
def _secret(name):
    return os.path.join(os.environ.get("QUEST_SECRETS_DIR", "/secrets"), name)
...
class WebRTCHeadset:
    def __init__(
        self,
        serviceAccountKeyFile=_secret('serviceAccountKey.json'),
        signalingSettingsFile=_secret('signalingSettings.json'),
        video_buffer_size=1,
        data_buffer_size=1,
        send_data_freq=10,
    ):        
